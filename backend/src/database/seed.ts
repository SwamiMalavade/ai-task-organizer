...
async function seedDatabase() {
  ...
  try {
    ...
    await connection.query('DELETE FROM raw_notes');
    ...
    const sampleRawNotes = [
      {
        user_id: userIds[0],
        raw_text: 'Finish presentation by Friday, check AWS logs for errors, call client about project update, review Q4 reports'
      },
      {
        user_id: userIds[1],
        raw_text: 'Team meeting tomorrow at 10am, code review needed for auth module, submit expense report by EOD'
      },
      {
        user_id: userIds[2],
        raw_text: 'Fix production bug ASAP, update documentation, schedule 1-on-1 with manager, buy groceries'
      }
    ];
